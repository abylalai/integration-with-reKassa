...
if (
	(date("G") >= 7 && date("G") <= 9)			// С 06:00 до 09:00
	//|| date("Y-m-d") == '2022-05-12'
)
{
	$lang="ru";
	...
	if ($stop == 1)
	{
		...
		{		
            foreach($s as $r){
                ...
                ->setAutoTicketToNextPage(true) # автоматический переключатся на вторую страницу тикетов
                ...
                $result = $rekassa->Integration->ticketsToSQL();
		...
		else
		{
			...
			$updateD = $mysqli->query($sql_updateD);
            $upd = $mysqli->query('UPDATE `i_rekassa_info` SET `check_date` = \'0\' WHERE `check_date` = 1');
			echo 'End today!';
		}
	}
}
