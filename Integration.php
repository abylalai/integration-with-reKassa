...
class RekassaAPI
{
	...
	{	
		...
		{
			$info["access_token"] = $this->curl->response;						
	...
	public function createNewTicket($id_cash, $summa = 0, $ticket_type){
		...
		$this->curl->POST($url, $header, $request);

		$response = $this->curl->response;
		$http_code = $this->curl->http_code;
		if ($http_code == 200)
		{
			$result = json_decode($response, true);	
		}
		...
		$info["http_code"] = $http_code;
		$info["curl_error"] = $this->curl->error;
		$info["result"] = $result;
		$info["url"] = $url;		
